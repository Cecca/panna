...
if __name__ == "__main__":
    ...
    for path in paths:
        ...
        # COmpute times for 1% and 10% of the data
        for fraction in [0.01, 0.1, 0.25]:
            n_samples = int(data.shape[0] * fraction)
            data_subset = data[:n_samples]

            emst = panna.EMST(data_subset, delta=0.01, epsilon=0, family="e2lsh")
            start_time = perf_counter()
            emst.find_mst()
            end_time = perf_counter()
            elapsed_time = end_time - start_time

            emst = panna.EMST(data_subset, delta=0.01, epsilon=0.2, family="e2lsh")
            start_time = perf_counter()
            emst.find_mst()
            end_time = perf_counter()
            elapsed_time2 = end_time - start_time
            print(f"Finished dataset {path}, fraction {fraction}. Times: K+ {elapsed_time}, K± e0.2 {elapsed_time2}")
            ...
            #         f_out.write(f"K+, {data_subset.shape[0]}, {path}, 0, {elapsed_time}\n")
            #         f_out.write(f"K± e0.2, {data_subset.shape[0]}, {path}, 0, {elapsed_time2}\n")
            #         f_out.flush()